...
const ModeratorNavigation = () => {
    return (
        <>
            ...
            <NavLink href={route('candidate')} active={route().current('candidate')} className={`${route().current('candidate')}`}><FaUserTie className='h-5 w-5 me-1' />
                Candidate</NavLink>

            <NavLink href={route('ballot')} active={route().current('ballot')} className={`${route().current('ballot')}`}><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5 me-1">
