...
const ModeratorNavigation = () => {
    return (
        <>
            ...
            <NavLink href={route('votes')} active={route().current('votes')} className={`${route().current('votes')}`}>
            ...
            <NavLink href={route('partylists')} active={route().current('partylists')} className={`${route().current('partylists')}`}>
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5 me-1">
            ...
            <NavLink href={route('positions')} active={route().current('positions')} className={`${route().current('positions')}`}>
            ...
            <NavLink href={route('candidate')} active={route().current('candidate')} className={`${route().current('candidate')}`}>
                <LiaUserTieSolid className='h-6 w-6 me-1' />
                Candidate
            </NavLink>

            <NavLink href={route('ballot')} active={route().current('ballot')} className={`${route().current('ballot')}`}>
